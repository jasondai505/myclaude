...
```powershell
# 框架零：统一调度
python orchestrator.py close                             # 收盘流水线（采集→复盘→简报）
python orchestrator.py pre                               # 盘前流水线（采集→公众号→建议）
...
### dashboard 仪表盘 (2026-06-04 新建)

```
dashboard/
  app.py                      # Streamlit 首页 — 今日概览+市场指数+操作建议摘要+BOM热点+流水线状态
  pages/
    2_📊_报告浏览.py           # 历史报告浏览（复盘/advice/BOM/公众号），支持日期筛选+全文查看
    3_🚀_流程触发.py           # 手动触发流水线（复盘/advice/BOM/采集/公众号），后台运行+实时日志
    4_📈_监控状态.py           # 数据新鲜度监控+流水线状态+系统配置一览
    5_🔍_个股查询.py           # 个股行情/BOM产业链位置/深度分析报告，支持自选股快捷选择
  utils/
    __init__.py               # 公共 API re-export
    data_bridge.py            # 数据桥接层 — 对现有模块只读封装，不修改任何现有代码
  logs/                       # 流水线运行日志 (pipeline_timestamp.log)
  run_dashboard.bat           # 一键启动 (streamlit run --port 8501)
```

**设计原则**：dashboard 是对现有模块的**只读消费层**，不修改 `daily_review/`、`bom_analyzer/`、`stock_deep/` 的任何代码。所有数据通过 `data_bridge.py` 统一访问。
...
8. **长会话后主动反思**：累计 ≥5 次工具调用或 ≥3 次 commit 后，主动总结本次会话的踩坑和发现。分两类处理：
   - **结构性规则**（铁律/方法论，影响后续行为）→ 先总结展示 → 用户确认 → 写入 CLAUDE.md
   - **操作备忘录**（文件路径/三板斧/踩坑记录）→ 直接追加到 LESSONS.md，用户事后 git diff 审查
   - CLAUDE.md 是宪法（自动注入），LESSONS.md 是工作笔记（AI 需要时 Read）
...
- **聚焦池批量拉取三板斧**（`_parallel_stock_fetch`）：①研报 → 人气前50或涨停标的，5 并发 ②盈利预测 → 同上但排除自选股（已拉过），5 并发 ③互动+新闻 → 聚焦池全部标的，8 并发。**禁止退化为逐只 sleep 循环**。详见 `run.py:_build_focus_pool_full`。
