"""产业链图谱 — ECharts 树图 · 跨链关联连线 · 点击标的详情面板"""
import json, sys, re
...
st.title("🔬 产业链图谱")
st.caption("L1 行业 → L2 层级 → L3 卡脖子节点 → L4 标的 · 跨链关联 + 点击详情")
...
with st.sidebar:
    ...
    # P0-3: support cross-chain jump via ?chain=xxx
    try:
    ...
    st.subheader("🔗 跨链关联")
    if cross:
        with st.expander(f"{len(cross)} 个跨链环节", expanded=True):
...
# ============================================================
# Build cross-chain graph data (P0-3)
# ============================================================
cross_graph = None
...
if chain_segments_in_cross:
    ...
    graph_nodes.append({
        "id": selected, "name": selected, "symbolSize": 36,
...
with col4:
    linked = len(chain_segments_in_cross)
...
<style>
  ...
  #cross-chart {{ width:100%; height:260px; margin-top:8px; }}
  .cross-title {{ text-align:center; color:#64748b; font-size:12px; margin-top:8px; }}
  #detail-panel {{
    position:fixed; top:0; right:-420px; width:400px; height:100vh;
    background:#0f172a; color:#e2e8f0; padding:20px; z-index:9999;
    transition:right 0.3s ease; overflow-y:auto;
    box-shadow:-4px 0 24px rgba(0,0,0,.4); font-size:13px;
  }}
  ...
  #detail-panel h2 {{ font-size:18px; margin:0 0 4px 0; }}
  #detail-panel .code {{ color:#64748b; font-size:12px; }}
  #detail-panel .price {{ font-size:28px; font-weight:700; margin:8px 0; }}
  #detail-panel .chg-up {{ color:#10b981; }}
  #detail-panel .chg-down {{ color:#ef4444; }}
...
<div id="chart"></div>
<div class="cross-title">{'🔗 跨链关联网络' if cross_graph else ''}</div>
...
// === Helper: walk tree and inject tooltip formatter ===
function walkTree(node) {{
    var m = node.tooltip_data || {{}};
    var extra = '';
    if (m.cross_ref) {{
        extra = '<br/>📎 点击跳转到「' + m.cross_ref + '」产业链';
    }} else if (m.code) {{
        extra = '<br/>' + m.name;
        ...
        if (m.market_cap > 0) extra += '  市值: ' + (m.market_cap>10000?(m.market_cap/10000).toFixed(0)+'万亿':(m.market_cap>0?(m.market_cap.toFixed(0)+'亿':'')));
...
treeChart.setOption({{
    ...
    series: [{{
        ...
        top: '2%', left: '6%', bottom: '2%', right: '18%',
        symbol: 'circle',
        symbolSize: function(v) {{ return v.value ? Math.min(28, 8+v.value*1.4) : 12; }},
...
treeChart.on('click', function(p) {{
    ...
    // P0-3: click cross-chain reference → switch chain
    if (m.cross_ref) {{
        var params = new URLSearchParams(window.parent.location.search);
        params.set('chain', m.cross_ref);
        window.parent.location.search = params.toString();
        return;
    }}
    // P0-4: click stock node → open detail panel
    if (m.code) {{
        openDetailPanel(m);
        return;
    }}
...
if (crossGraph) {{
    ...
    crossChart.setOption({{
        ...
        series: [{{
            type: 'graph', layout: 'force', force: {{ repulsion: 300, edgeLength: [120, 260] }},
            roam: true, draggable: true,
            data: crossGraph.nodes,
            links: crossGraph.links.map(function(l) {{
                return {{ source: l.source, target: l.target,
                         lineStyle: {{ color: '#475569', curveness: 0.2, width: 1 }} }};
            }}),
            categories: [
                {{ name: '当前链' }}, {{ name: '共享环节' }}, {{ name: '关联链' }}
            ],
    ...
    crossChart.on('click', function(p) {{
        if (p.dataType === 'node') {{
            var nid = p.data.id || p.name || '';
            if (nid.indexOf('seg_') === 0) return; // skip segment nodes
            if (nid !== '{selected}') {{
                var params = new URLSearchParams(window.parent.location.search);
                params.set('chain', nid);
                window.parent.location.search = params.toString();
            }}
        }}
    }});
...
function openDetailPanel(m) {{
    ...
    var capStr = m.market_cap > 10000 ? (m.market_cap/10000).toFixed(1)+'万亿'
               : (m.market_cap > 0 ? m.market_cap.toFixed(0)+'亿' : '--');

    content.innerHTML =
        '<h2>' + m.name + '</h2>' +
        '<div class="code">' + m.code + '</div>' +
        '<div class="price">' + (m.price ? m.price.toFixed(2) : '--') +
        ' <span class="' + chgCls + '" style="font-size:16px">' + (m.change_pct>=0?'+':'') + m.change_pct.toFixed(2) + '%</span></div>' +
        '<div class="metric-row">' +
          '<div class="metric"><div class="label">PE</div><div class="val">' + (m.pe>0?m.pe.toFixed(1):'--') + '</div></div>' +
          '<div class="metric"><div class="label">PB</div><div class="val">' + (m.pb>0?m.pb.toFixed(1):'--') + '</div></div>' +
          '<div class="metric"><div class="label">市值</div><div class="val">' + capStr + '</div></div>' +
        '</div>' +
        '<div style="margin-top:16px;font-size:14px;font-weight:600">FEV 评分</div>' +
        '<div class="fev-bar">' +
          '<div class="fev-item"><span>F</span><div class="bar" style="background:#3b82f6;width:'+(m.f_score*10)+'%"></div><span style="font-size:11px">'+m.f_score+'/10</span></div>' +
          ...
          '<div class="fev-item"><span>FEV</span><div class="bar" style="background:#8b5cf6;width:'+(m.fev/30*100)+'%"></div><span style="font-size:11px;font-weight:700">'+m.fev+'/30</span></div>' +
        '</div>';
    ...
    if (hist.length >= 2) {{
        if (!sparkChart) sparkChart = echarts.init(document.getElementById('sparkline'));
        sparkChart.setOption({{
            ...
            series: [{{
                type: 'line', data: hist.map(function(h){{return h.fev;}}),
                ...
                markLine: {{ silent: true, symbol: 'none',
                    data: [{{ yAxis: 15, lineStyle: {{ color: '#f59e0b', type: 'dashed', width: 1 }} }}],
                    label: {{ formatter: '15', fontSize: 9, color: '#f59e0b' }}
                }}
            }}]
        }});
...
window.addEventListener('resize', function() {{
    treeChart.resize();
    if (crossGraph) echarts.getInstanceByDom(document.getElementById('cross-chart')).resize();
}});
</script>
</body></html>"""

st.components.v1.html(html, height=1020 if cross_graph else 760, scrolling=False)
