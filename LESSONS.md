...
**待解决**:
- G2/G3 数据稀疏 → 需要更多催化/星球覆盖，或降低阈值 (当前 ≥6)
- zsxq 代码格式: 5 位 vs 6 位不统一 (如 "026051" vs "002605")
- 双轨筛选阈值需要根据数据密度迭代调整
- 档案重跑: 338 份已有档案不含 G-Factor 章节，需 `stock_dossier_builder.py --pool` 重生成

**架构**: 格雷厄姆轨 (FEV) + 费雪轨 (G-Factor) 双轨并行，不可公度维度不合成。不是调权重，是分席位。
