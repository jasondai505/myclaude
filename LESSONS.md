...
- ✅ ~~公告深研管线不稳定~~ — 已完成 2026-06-24（幂等改为 ≤3天不跳过，INSERT OR REPLACE 防重复）
- ✅ ~~catalyst_tracker C 轨确认加分~~ — 已完成 2026-06-23（`_build_selection` C轨排序 +15分确认加成）
- ✅ ~~情绪跟踪三源合并~~ — 已完成 2026-06-24（调研×0.8/互动×0.7/业绩×1.0 统一评分，按股票聚合）
...
### Haiku 提取的不一致性
同一批数据两次跑，氧化钇有时抓到（#3, 66分）有时漏。Haiku 变异性约 10-20%。✅ 已做：batch_size 10→15 + supply_shock/price_spike prompt 加权（2026-06-24）。待 Phase 5：高价值源两轮取并集。
