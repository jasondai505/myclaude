...
def render_report(
    ...
    *,
    ...
    if history:
        ...
        lines.append(_row("涨停", "limit_up_count"))
        lines.append(_row("连板≥2", "limit_up_2plus"))
        lines.append(_row("跌停", "limit_down_count"))
        lines.append("")
    ...
    for label, data in market.get("indices", {}).items():
        ...
        amount = data.get("amount_wan", 0) / 10000  # 万→亿
    ...
    if sentiment:
        ...
        def _render_classified_table(title: str, stocks: list[dict]) -> None:
            if not stocks:
                return
        ...
        _render_classified_table("逻辑驱动涨停", sentiment.get("logic_stocks", []))
        _render_classified_table("混合涨停", sentiment.get("mixed_stocks", []))
        _render_classified_table("情绪驱动涨停", sentiment.get("emotion_stocks", []))
    ...
    def _render_theme_block(t, stocks):
        ...
        lines.append(_theme_block_amount_summary(show_stocks))
        lines.append("")
        _zt = zt_pool or {}
    ...
    # 纯人气/中期强势新方向（不在涨停题材分级内；level 0 异动，不写历史）
    if theme_new_dirs:
    ...
    if focus_pool_data:
        ...
        def _render_focus_table(items, max_n=15):
        ...
        if resonance:
            ...
            _render_focus_table(resonance)

        if hot_surge:
            lines.append("### 人气飙升标的\n")
            _render_focus_table(hot_surge, 10)

        if watch_items:
            lines.append("### 自选股状态\n")
            _render_focus_table(watch_items, 30)
