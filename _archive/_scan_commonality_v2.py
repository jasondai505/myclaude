...
if not (Path(__file__).parent / "daily_review").exists():
    sys.path.insert(0, str(Path(__file__).parent.parent / "daily_review"))
...
# 缓存 & 数据路径
# ============================================================
_PROJECT = Path(__file__).parent if (Path(__file__).parent / "daily_review").exists() else Path(__file__).parent.parent
CACHE_DIR = _PROJECT / "daily_review" / "data" / "commonality_cache"
CACHE_DIR.mkdir(parents=True, exist_ok=True)
MULTI_MAP_PATH = _PROJECT / "daily_review" / "data" / "multi_concept_map.json"
